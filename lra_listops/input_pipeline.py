...
#  tf.logging.info('Finished processing vocab size={}'.format(len(vocab_set)))
  ...
  def tokenize(d):
    return ( tf_encode(d['Source'])[:max_length], d['Target'] )
...
#  num_elements = 0
#  for indexo,element in enumerate(train_dataset):
#    if indexo % 1000 ==0:
#        print(indexo)
#    num_elements += 1

#  print("elements train_ds",train_dataset)
#  sys.exit()
...
#  max_shape = {'inputs': [max_length], 'targets': []}
  ...
  max_shape = [max_length]

  train_dataset = train_dataset.shuffle(
      buffer_size=1024, reshuffle_each_iteration=True).padded_batch(
          batch_size, padded_shapes=([max_length], []) )
  val_dataset = val_dataset.padded_batch(batch_size, padded_shapes=([max_length], [])  )
  test_dataset = test_dataset.padded_batch(batch_size, padded_shapes=([max_length], [])  )
